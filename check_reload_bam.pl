#!/usr/bin/perl
use warnings;
use strict;

#dir check
my $pwd=`pwd`;chomp $pwd;
if($pwd ne "/Volumes/cancer/kaz_gdc"){die "!!ERROR!! doing on wrong dir\n";}

my $token=`cat gdc-user-token.txt`;
my $bp=$ARGV[0];
my $bamdir="$bp/$bp"."_norm_cancergenes_bam";
my $manifest = `ls $bp|grep '^gdc_manifest'`;chomp $manifest;

use Parallel::ForkManager;
my $pm = new Parallel::ForkManager(10);

open(MANI,"$bp/$manifest");
my $devnull=<MANI>;
my $linen=0;
while(<MANI>){
		$linen++;
		#forks and returns the pid for child
		my $pid = $pm->start and next;
		
		chomp ;
		my @line=split(/\t/,);
		print "$linen:$line[1] doing\n";
		my $taila=`samtools view $bamdir/$line[1] |tail -n 1`;
		if(!defined $taila){$taila="0\t0\t0";}
		my @taila=split(/\t/,$taila);
		if(($taila[2] ne "chrX")&&($taila[2] ne "chrY")){
				print "$linen:$line[1] chr $taila[2] eq not chrXorY so download again\n";
				my $focal=0;
				while($focal==0){
						system("curl --header \"X-Auth-Token: $token\" --request POST https://gdc-api.nci.nih.gov/slicing/view/$line[0] --header \"Content-Type: application/json\" -d\@top_driverallexon_json.txt --output $bamdir/$line[1] > /dev/null 2>&1");
						my $tailb=`samtools view $bamdir/$line[1] |tail -n 1`;
						my @tailb=split(/\t/,$tailb);
						if(!defined $tailb){$taila="0\t0\t0";}
						if(($tailb[2] ne "chrX")&&($tailb[2] ne "chrY")&&($taila ne $tailb)){$taila=$tailb;
						}else{
								$focal++;
								print "$linen:$line[1] redownloaded $tailb[2] is ok\n";
								}
				}
		}

		$pm->finish; #terminates the child process
}

