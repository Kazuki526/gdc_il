...
my $out_dir;
if(!defined $ARGV[0]){$out_dir=`pwd`;chomp $out_dir;
}else{$out_dir=$ARGV[0];}
...
open (OUTV,">$out_dir/exac_nontcga_liftovered.vcf");
open (OUTT,"|gzip -c >$out_dir/nontcga_liftovered_checked_likevcf.tsv.gz");
print OUTT "chr\tstart\tref\talt\t".join("\t",(@info_ac,@info_an))."\n";
foreach my $chr (@chr){
		print "liftover $chr\n";
		my %remap = map{chomp;my @c=split("\t");$c[0]=~s/^chr//;($c[3],"$c[0]:$c[1]-$c[2]")}`~/vep/samtools/bin/liftOver $out_dir/before_liftover_bed/$chr.bed ~/.vep/hg19ToHg38.over.chain /dev/stdout /dev/null 2>/dev/null`;
		print "printing out $chr \n";
		open (IN,"gunzip -c $out_dir/before_liftover/$chr.vcf.gz|");
		while(<IN>){
				if($_=~/^#/){
...
open (OUTV,">$out_dir/exac_nontcga_liftovered_indel.vcf");
open (ERR,"|gzip -c >$out_dir/exac_nontcga_liftovererror_indel.vcf.gz");
open (OUTT,"|gzip -c >$out_dir/nontcga_liftovered_checked_likevcf_indel.tsv.gz");
print OUTT "chr\tstart\tref\talt\t".join("\t",(@info_ac,@info_an))."\n";
foreach my $chr (@chr){
		print "liftover $chr\n";
		my %remap = map{chomp;my @c=split("\t");$c[0]=~s/^chr//;($c[3],"$c[0]:$c[1]-$c[2]")}`~/vep/samtools/bin/liftOver $out_dir/before_liftover_bed_indel/$chr.bed ~/.vep/hg19ToHg38.over.chain /dev/stdout /dev/null 2>/dev/null`;
		print "printing out $chr \n";
		open (IN,"gunzip -c $out_dir/before_liftover_indel/$chr.vcf.gz|");
		while(<IN>){
				if($_=~/^#/){
						if($chr eq "1"){print OUTV "$_";}
						next;
