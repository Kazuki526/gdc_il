...
#### onocogene TSGにわけてuk10k,cancer のAFをプロット
##(下のscale__logを消すとlog表示でないのも作成可)
all_variant %>>%
...
#### truncate mutationのAFのみをプロット(oncogene,TSGに分けて)
##こちらもscale__logを消すと
all_variant %>>%
  left_join(driver_genes%>>%dplyr::select(gene,role),by=c("gene_symbol"="gene")) %>>%
...
#### mutationをsilent, missenseに分けて差を探そうと
#### uk10k,cancerのそれぞれのmutationをAFでプロット
##こちらもscale__log消すと
all_variant %>>%
# filter(uk_ac >5) %>>%
  left_join(driver_genes%>>%dplyr::select(gene,role),by=c("gene_symbol"="gene")) %>>%
...
##########################################################################################################
##########################################################################################################
#testで作られたall_variantはそのまま使用!
#somatic_mutationで作られたmaf_somaticもそのまま使用!

############ 各遺伝子ごとにsomatic germline mutationの分布 #############
#作図用のcdsのregionを抽出
topdriver_cds_bed=read_tsv("~/git/driver_genes/onlytop105/top_driver105cds.bed",col_types = "ciicic")%>>%
